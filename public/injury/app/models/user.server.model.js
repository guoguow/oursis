...
Pay.getindex=  function  get(a,b,c,d,tablename1,tablename2,condition,callback) {
    ...
    var sql = "select a."+a+"  dm,sum(a."+b+") pm,je,rm  from " +tablename1+" a,(select max(b."+d+" )  rm, sum("+c+") je  from "+tablename2+ " b  where "+condition.name+" ='"+condition.value+"' ) b where a."+condition.name+" = "+condition.value +" ORDER BY a."+a+" desc limit 1 ";
    console.log(sql);
    client.getDbCon(sql,function(err,results,fields){
        if(err){
            throw err;
